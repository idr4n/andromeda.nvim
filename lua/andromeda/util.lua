...
function M.blend_bg(hex, amount, bg)
  bg = bg or M.bg
  if not hex or not bg or hex == "NONE" or bg == "NONE" then
    return hex or bg or "#000000"
  end
  return M.blend(hex, amount, bg)
end
...
function M.blend_fg(hex, amount, fg)
  fg = fg or M.fg
  if not hex or not fg or hex == "NONE" or fg == "NONE" then
    return hex or fg or "#ffffff"
  end
  return M.blend(hex, amount, fg)
end
...
function M.brighten(color, lightness_amount, saturation_amount)
  ...
  local hsluv = require("andromeda.hsluv")
